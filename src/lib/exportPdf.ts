...
export async function generatePDF(data: ReportData): Promise<void> {
  ...
  const accentColor = { r: 234, g: 179, b: 8 };    // Laranja/Amarelo
  const borderColor = { r: 0, g: 0, b: 0 };        // Preto
  const darkTextColor = { r: 30, g: 41, b: 59 };   // Cinza Escuro
  ...
  const drawPage = (pageNum: number, photosOnPage: typeof data.photos) => {
    ...
    // --- VARIÁVEIS DE DIMENSÃO DO CABEÇALHO ---
    const titleRowY = pageAreaY + 4;        // Título "RELATÓRIO FOTOGRÁFICO"
    const infoBlockY = pageAreaY + 12;       // Início do bloco da Grade/Borda Geral
    
    // Altura total da Grade definida para 45mm (15mm por linha)
    const infoBlockHeight = 45;             
    const innerPadding = 2;                 // Margem interna para texto horizontal
    
    // Altura das linhas da grade
    const rowHeight = infoBlockHeight / 3; // 15mm por linha

    // AJUSTE CRÍTICO: Offset para centralização vertical (9.5mm é o ponto ideal para centralizar visualmente)
    const rowCenterOffset = 9.5; 
    
    // Larguras das colunas
    const col1Width = 40;                   // Coluna 1: Logo (fixa)
    const col3Width = 40;                   // Coluna 3 (Vermelha): Fixa em 40mm
    const col2Width = contentWidth - col1Width - col3Width; // Coluna 2 (Verde): Restante
    ...
    const renderCellContent = (colNum: number, yStart: number, label: string, value: string, maxCellWidth: number, maxLines: number = 1, alignRight: boolean = false, valueOffsetX: number = 2) => {
        ...
        if (colNum === 2) {
            ...
            // O slice(0, maxLines) garante que o Objetivo use até 2 linhas.
            pdf.text(splitValue.slice(0, maxLines), valueX, y); 
        
        // Coluna 3 (VERMELHA): Label à Esquerda, Valor à Direita
        } else if (colNum === 3) {
            
            const valueX_Right = pageWidth - margin - innerPadding;
            
            // Renderiza o valor com alinhamento à direita (mesmo Y)
            pdf.text(valueText, valueX_Right, y, { align: 'right' });
        }
    };
    
    // --- CONTEÚDO DE DADOS DA GRADE (USANDO NOVO OFFSET) ---
    
    // LINHA 1 (Topo) - Spacing Default (2mm)
    ...
    // LINHA 2 (Meio) - Spacing AUMENTADO (4mm)
    const cellY2 = row1Boundary; 
    renderCellContent(2, cellY2, 'Título do Relatório', data.config.tituloRelatorio, col2Width, 1, false, 4); // <--- Spacing 4mm
    renderCellContent(3, cellY2, 'Data', date, col3Width, 1, true);

    // LINHA 3 (Fundo) - Spacing Default (2mm)
    ...
    // --- GRID DE FOTOS (2x2) CENTRALIZADO ---
    
    const photosStartY = infoBlockY + infoBlockHeight + 15; 
    ...
    photosOnPage.forEach((photo, index) => {
      ...
      const labelY = photoY + photoHeight + 5;
      ...
      const splitDesc = pdf.splitTextToSize(description, descriptionMaxWidth);
      
      // Primeira linha da descrição
      pdf.text(splitDesc.shift() || '', descriptionStartX, labelY);
      
      // Adiciona a segunda linha da descrição, se houver
      if (splitDesc.length > 0) {
          pdf.text(splitDesc.shift() || '', photoX, labelY + 4);
      }
    });
